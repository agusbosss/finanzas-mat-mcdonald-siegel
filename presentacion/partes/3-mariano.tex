...
\begin{frame}{Desguace óptimo del proyecto}
  El modelo es \concepto{versátil}: la misma lógica que decide cuándo adquirir
  ...
  \rj{$c^*$}. Con vida finita harían falta \resultado{métodos numéricos}.
\end{frame}
...
\begin{frame}{Saltos en $V_t$: el riesgo de ruina}
  ...
  \concepto{vencimiento incierto}. El riesgo se absorbe sumando \az{$\lambda$}
...
\begin{frame}{Impacto de la obsolescencia en la regla}
  ...
  \begin{itemize}
    \item Más \az{$\lambda$} \resultado{reduce el valor de la opción} y baja el
          umbral \rj{$C^*$}.
