...
\begin{frame}{Aplicación del CAPM al riesgo}
  ...
  \begin{ec}
    \rj{\mu} = \az{r + \varepsilon(\hat{\alpha}_v - r)
      ...
      = \az{\varepsilon\hat{\alpha}_v + (1-\varepsilon)\hat{\alpha}_f}
  \end{ec}
\end{frame}
...
\begin{frame}{La regla de inversión: sin y con incertidumbre}
  ...
  \begin{ecmed}
    ...
    \text{\small invertir si } \az{V_0\delta_v} \geq \az{F_0\delta_f}
  \end{ecmed}
  \vspace{-1mm}
  {\small\noindent Válido solo si $\az{\delta_v} < \az{\delta_f}$.}
  \par\vspace{1mm}
  \concepto{Con incertidumbre} ($\az{\sigma^2} > 0$): la regla incorpora un
  término extra \az{$h$}, el valor de esperar:
  \begin{ecmed}
    \az{V_0\,\delta_v} \geq \az{F_0\,\delta_f + h(V_0, F_0)}, \qquad \az{h} > 0
  \end{ecmed}
