...
\begin{frame}{Modelo general: $F$ también estocástico}
  Ahora \az{$F_t$} también es aleatorio y sigue un MBG. La caracterización
  del umbral ya no es evidente, pero la regla sigue siendo de umbral sobre el
  ratio \rj{$V$}$/$\az{$F$}.
  \par\vspace{3mm}
  \concepto{Homogeneidad de grado cero}: con \rj{$V'=kV$} y \az{$F'=kF$} el
  problema es formalmente idéntico, de modo que el umbral no depende de $k$
