...
% ============================================================
\divisoria{Agustín Bossart}% TEMPORAL: borrar en la versión final
...
\begin{frame}{Aplicación del modelo con ejemplos numéricos}
  Todas las tablas se evalúan en el punto \rj{$V = F = 1$}: el primer punto
...
\begin{frame}{Estáticas comparativas de las Tablas I y II}
  \begin{itemize}
    ...
    \item \rj{$\delta_v$} $\uparrow$\;: valor $\downarrow$, \rj{$C^*$}
          $\downarrow$ --- el efecto más fuerte; para
          \rj{$\delta_v \to 0$}, $X \to V$ y $C^* \to \infty$.
...
\begin{frame}{El monopolista: $V$ y $\delta_v$ endógenos}
  ...
  \begin{ecmed}
    \rj{V(\theta_0)} = \az{\frac{B\,\theta_0^{\gamma}}{\bar{\alpha}_v - \alpha_v}},
    \qquad
    \az{\alpha_v = \gamma\alpha_\theta + \tfrac{1}{2}\gamma(\gamma-1)\sigma_\theta^2},
    \qquad
    \az{\bar{\alpha}_v = r + \phi\,\rho_{\theta m}\,\gamma\,\sigma_\theta}
  \end{ecmed}
  \cajaec{\rj{\delta_v} = \az{\bar{\alpha}_v - \alpha_v}
...
% (guion §5 y §6 — asteriscos y contraejemplo)
\begin{frame}{Los asteriscos de la Tabla IV}
  Al variar \az{$\sigma_\theta$} se mueven a la vez \az{$\sigma_v$},
  \az{$\alpha_v$} (Jensen) y \az{$\bar{\alpha}_v$} (CAPM). En la celda
  $\sigma_\theta^2 = 0.20$, $\alpha_\theta = 0$, $\rho_{\theta m} = 0$:
  \begin{ec}
    \rj{\delta_v} = \underbrace{\az{0.05}}_{r}
      + \underbrace{\az{0}}_{\text{CAPM}}
      - \underbrace{\az{0}}_{\gamma\alpha_\theta}
      - \underbrace{\az{0.075}}_{\frac{1}{2}\gamma(\gamma-1)\sigma_\theta^2}
      = \rj{-0.025} < 0
  \end{ec}
  El término de Jensen supera al retorno requerido: esperar siempre suma y
  la inversión nunca se concreta.
  \par\vspace{2mm}
  Con $\eta = 1.0001$, $\alpha_\theta = -0.03$ y $\rho_{\theta m} = 0.95$:
  subir \az{$\sigma_\theta^2$} de $0.0025$ a $0.0050$ \emph{reduce} la
  opción de \rj{$0.0293$} a \rj{$0.0286$}.
\end{frame}
...
\begin{frame}{Conclusiones}
  \begin{cajatexto}
    ``El valor perdido por adoptar subóptimamente un proyecto de VAN cero
    está fácilmente entre el \textbf{10 y el 20 por ciento} (o más) del
    valor del proyecto.''
  \end{cajatexto}
  \vspace{2mm}
  Limitaciones señaladas por los autores:
  \begin{enumerate}
    \item \concepto{MBG}: plausible para valores presentes; con payouts y
          varianzas dependientes de los precios, las opciones serían
          menores.
    \item \concepto{Inversión indivisible}: si se pudiera invertir
          gradualmente, el problema del timing perdería importancia.
    \item \concepto{Irreversibilidad total}: se ignora el desarme posterior
          del proyecto; la reversibilidad entra implícitamente vía
          depreciación.
  \end{enumerate}
\end{frame}
